...
/** Cached session data — captured once in reconstructTodoState, reused after. */
let cachedSessionId: string | null = null;
let cachedBranch: any[] | null = null;
...
export function reconstructTodoState(ctx: {
	...
	// Cache session data on first capture — subsequent calls reuse it
	// to avoid accessing a potentially stale ctx.sessionManager.
	if (cachedSessionId === null) {
		cachedSessionId = ctx.sessionManager.getSessionFile() ?? "ephemeral";
	}
	if (cachedBranch === null) {
		cachedBranch = ctx.sessionManager.getBranch();
	}
	activeSessionId = cachedSessionId;
	const state = getSessionState(cachedSessionId);
	state.tasks = [];
	state.nextId = 1;
	for (const entry of cachedBranch) {
		if (entry.type !== "message") continue;
		const msg = entry.message;
		if (msg.role !== "toolResult" || msg.toolName !== "todo") continue;
		const details: unknown = msg.details;
		if (!isTaskDetails(details)) continue;
		state.tasks = details.tasks.map((t) => ({
			...t,
			blockedBy: t.blockedBy ? [...t.blockedBy] : undefined,
